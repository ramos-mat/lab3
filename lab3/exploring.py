...
def find_best_point(im, possible_points : list, robot_loc):
    ...
    for pt in possible_points:
        for ix in range(-1, 2):
            for iy in range(-1, 2):
                ...
                # Store the local counts first so the scoring step below stays simple.
    ...
    # When possible, skip tiny frontiers right next to the robot and make visible progress.
...
def find_waypoints(im, path):
    ...
    waypoints = [path[0]]
    last_kept_index = 0
    # Keep corners, but drop a lot of points along long straight sections.
...
# -------------- Small checks used by the file test ---------------
